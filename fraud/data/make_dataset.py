...
def download_and_load_new(path="./", head_num=None):
    '''Download and load new URL dataset.'''
    download_new(path)
    return load_new(path, head_num=head_num)

def download_new(path="./"):
    '''Download new URL dataset to selected path.'''
    ...
    # Assign the Kaggle data set URL into variable
    dataset = "https://www.kaggle.com/datasets/sid321axn/malicious-urls-dataset/data"

    # Using opendatasets let's download the data sets as zip file
    return od.download(dataset, path)

def load_new(path="./", head_num=None):
    '''Load new URL dataset from selected path showing first n rows.'''
...
def upload_new(file, path="./"):
    '''Upload processed new URL dataset.'''
